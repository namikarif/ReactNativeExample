            ...
            if (maskType === 'currency') {
                if (value !== undefined && value.length < val.length) {
                    if (val.includes(delimeter)) {
                        let newQuantityArray = val.split(delimeter);
                        let latest = newQuantityArray[newQuantityArray.length - 1];
                        newQuantityArray.splice(newQuantityArray.length - 1, 1);
                        val = newQuantityArray.join('') + deciamlSperator + latest;
                    }
                    if (val.includes(deciamlSperator)) {
                        let newQuantityArray = val.split(deciamlSperator);
                        let latest = newQuantityArray[newQuantityArray.length - 1];
                        newQuantityArray.splice(newQuantityArray.length - 1, 1);
                        val = newQuantityArray.join('') + deciamlSperator + latest;
                        let intVal = val.split(deciamlSperator)[0].replace(/[,.]/g, '');
                        let decimalValue = val.split(deciamlSperator)[1];
                        if (intVal.length > 3) {
                            let arr: string[] = [];
                            ...
                            intVal = intVal + arr.join('');
                        }

                        val = intVal + deciamlSperator + decimalValue;

                        let decimalPlaces: number =
                            maxDecimalPlaces !== undefined ? maxDecimalPlaces : 2;

                        if (
                            val.split(deciamlSperator)[1] !== undefined &&
                            value.split(deciamlSperator)[1] !== undefined &&
                            val.split(deciamlSperator)[1].length > value.split(deciamlSperator)[1].length &&
                            value.split(deciamlSperator)[1].length === decimalPlaces
                        ) {
                            return;
                        }
                        if (val.split(deciamlSperator)[1].length > decimalPlaces) {
    ...
    return (
        <TouchableWithoutFeedback onPress={setFocus} onLayout={onLayout}>
            <View style={{flexDirection: 'row'}}>
                ...
                <View style={[containerStyles, customContainerStyles]}>
                    ...
                    <View style={{flex: 1, flexDirection: 'row'}}>
                        ...
                        <TextInput value={value}
                                   ...
                                   onPress={onPress !== undefined ? onPress : handleFocus}
                        ...
                        />
                        {isPassword && (
                            <TouchableOpacity style={toggleButton}
